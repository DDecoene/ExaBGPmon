...
from models import bgp_updates, bgp_peers, adv_routes, bgp_config
from pymongo import DESCENDING
from requests import ConnectionError
from tasks import announce_route, withdraw_route, exabpg_process, is_exabgp_running, send_exabgp_command, \
    build_config_file
...
def peer(peer_id):
    ...
    if route_form.validate_on_submit():
        ...
        if adv_routes.find_one(
                {'peer': peer['ip'], 'prefix': route_form.prefix.data, 'next-hop': route_form.next_hop.data}):
    ...
    if peer_form.validate_on_submit():
        ...
        return render_template('peer_info.html', peer=peer, route_form=route_form, peer_form=peer_form,
...
def logs():
    ...
    if not peer:
        updates = list(
            bgp_updates.find({'type': {'$nin': exclude}}, {'_id': False}).sort('time', DESCENDING).limit(limit))
    else:
        updates = list(bgp_updates.find({'type': {'$nin': exclude}, 'peer': {'$eq': peer}}, {'_id': False}).sort('time',
                                                                                                                 DESCENDING).limit(
            limit))
